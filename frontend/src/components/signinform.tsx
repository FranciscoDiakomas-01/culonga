  ...
  async function handelOnSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    if (showPutCode) {
      ...
      if (res?.found && res?.token?.length > 0) {
        ...
        toast.warning(res?.message || "Código inválido");
      }
    ...
    if (!name || !lastname || !telefone || !password || !email) {
    ...
    const signIn: any = await createUser.sigIn({
      email,
      lastname,
      name,
      password,
    ...
    const serverMessage = Array.isArray(signIn?.message)
      ? signIn.message[0]
      : signIn?.message;

    toast.info(serverMessage || "Processado!");
      ...
      <div className="grid gap-6">
        <span className="grid gap-6 w-full lg:grid-cols-2">
          <div className="grid gap-3">
            ...
            <Input id="text" name="name" type="text" placeholder="seu nome" required />
          </div>

          <div className="grid gap-3">
            <Label htmlFor="text">Sobrenome</Label>
            <Input id="text" name="lastname" type="text" placeholder="seu sobrenome" required />
          </div>
